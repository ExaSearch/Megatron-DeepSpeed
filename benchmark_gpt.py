...
class DummyGPTDataset(GPTDataset):
    def __init__(
        self,
        name,
        num_samples,
        ...
        self.text = np.array([1111] * seq_length, dtype=np.int64)
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    # )
    train_ds = DummyGPTDataset(
        name="train",
        num_samples=train_val_test_num_samples,
        seq_length=args.seq_length,
        seed=args.seed,
    )
    # print_rank_0(f"> datset length: {train_ds.num_samples}")
    # print_rank_0(f"> datset length: {len(train_ds)}")
